...
def create_network():
    w_conv1 = weights_variable([8, 8, 4, 32])  # 卷积核8x8x4,输出32
    b_conv1 = bias_variable([32])

    w_conv2 = weights_variable([4, 4, 32, 64])  # 卷积核4x4x32,输出64
    b_conv2 = bias_variable([64])

    w_conv3 = weights_variable([3, 3, 64, 64])  # 卷积核3x3x64,输出64
    ...
    # 下面公式n为输入像素，p为padding：SAME为填充：值为floor((f-1)/2)，VAlID为不填充：值为0，f为卷积核大小：8x8就是8，s为步长strides
    # 卷积公式(n+2p-f)/s+1，第一次P为floor((8-1)/2)=floor(3.5)=3,第二次P为floor((4-1)/2)=floor(1.5)=1,第三次为floor((3-1)/2)=1
    h_conv1 = tf.nn.relu(conv2D(s, w_conv1, 4) + b_conv1)  # 代入公式(80+2x3-8)/4+1=floor(20.5)：20x20x32
    ...
    h_conv3 = tf.nn.relu(conv2D(h_conv2, w_conv3, 1) + b_conv3)  # 第三次卷积，卷积核是3x3和步长是1，像素不变：5x5x64
